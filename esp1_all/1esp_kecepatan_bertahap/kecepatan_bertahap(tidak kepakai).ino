...
void loop() {
    ...
    if (myGamepad && myGamepad->isConnected()) {
        ...
        // Jika target lebih rendah, turunkan akselerasi secara bertahap
        if (target_acc < current_acc) {
            current_acc -= 0.05;  // Sesuaikan nilai step sesuai kebutuhan
            if (current_acc < target_acc)
                current_acc = target_acc;
        } else {
            // Jika nilai target lebih tinggi, langsung naik ke nilai target
            current_acc = target_acc;
        }
